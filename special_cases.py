...
def special_cases(moc_name,moc_id):

    cond = False
    moc_list_gs = []
    for i in range(len(raw_data_df)):
        if  raw_data_df.iloc[i]["MOC"] == moc_name :
            cond  = True
            name = raw_data_df.iloc[i]["MOC"]
            if raw_data_df.iloc[i]['PARAMETER'] == moc_id:
                val = raw_data_df.iloc[i]['VALUES']
                break
    ...
    for i in range(len(raw_data_df)):
        if raw_data_df.iloc[i]["MOC"] == moc_name:
            if raw_data_df.iloc[i]['PARAMETER'] == moc_id:
                val = raw_data_df.iloc[i]['VALUES']
                moc_list_gs.append(raw_data_df.iloc[i]['MOC'] + "=" + val)
            raw_data_df.iloc[i]['MOC'] = raw_data_df.iloc[i]['MOC'] + "=" + val

    result = pd.DataFrame()

    for m in moc_list_gs:
        raw_moc = m
        moc = m[6:]

        df1 = (raw_data_df.loc[raw_data_df["MOC"] == raw_moc])
        df3 = df2.loc[(df2['MOC'] == moc)     &  (df2['HW/SW load'] == 'L18Q2G1G2')]       ####GET ALL ROWS FROM GOLD STANDARD FOR THIS PARTICULAR MOC AND LOAD


        my_list1 = df1["PARAMETER"].tolist()
        my_list2 = set(df3["Parameter"].tolist())
    ...
    if cond:
        ...
        #result = result.loc[(result['COMPARISION'] == 'F')]                         ### IF NEEDED ONLY GET THOSE ROWS WHERE WE GOT FALSE VALUE.
        #result.drop_duplicates(keep = 'first', inplace = True)
        #result.to_excel(writer, sheet_name = name)

moc_name_list = ['vsDataCapacityKey','vsDataDataRadioBearer','vsDataDrxProfile','vsDataFeatureKey',
                'vsDataIkePeer','vsDataIpAccessHostEt','vsDataIpHostLink','vsDataIpInterface',
                'vsDataIpSecTunnel','vsDataMACConfiguration','vsDataIpSystem','vsDataNonPlannedPciDrxProfile',
                 'vsDataOptionalFeatureLicense', 'vsDataPmFlexCounterFilter', 'vsDataQciProfilePredefined', 'vsDataRfPort',
                'vsDataIpAccessSctp','vsDataSignalingRadioBearer', 'vsDataVpnInterface']

moc_id_list = ['id','DataRadioBearerId','DrxProfileId','id',
                'userLabel','IpAccessHostEtId','IpHostLinkId','IpInterfaceId',
                'IpSecTunnelId','MACConfigurationId','IpSystemId','NonPlannedPciDrxProfileId',
                'userLabel','PmFlexCounterFilterId','QciProfilePredefinedId','RfPortId',
                'IpAccessSctpId','SignalingRadioBearerId','userLabel']

for i in range(len(moc_name_list)):
    special_cases(moc_name_list[i],moc_id_list[i])
